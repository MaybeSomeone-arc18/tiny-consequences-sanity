...
export default function Experience({story}:{story:Story}){
 ...
 return <main>
  <div className="top"><div className="seal">◌</div><div className="dept">THE DEPARTMENT OF<br/><strong>TINY CONSEQUENCES</strong></div><nav className="nav"><a href="#case">The case</a><a href="#model" onClick={()=>setShowModel(true)}>The structure</a></nav><span className="edition">CASE FILE 001 / A CITY OF ALMOSTS</span></div>
  <section className="intro"><div className="topographic" aria-hidden="true"/><p className="eyebrow">AN INTERACTIVE WORK OF FICTION · POWERED BY STRUCTURED CONTENT</p><h1><span>One</span><span>little thing</span><span><em>changes</em></span><span>everything.</span></h1><p>Follow a missing bench through a city. Every choice moves the story to a different place, and every turn has a reason you can inspect.</p><button className="model" onClick={()=>setShowModel(!showModel)}>{showModel?'CLOSE':'INSPECT'} THE CONTENT MODEL ↗</button></section>
  {showModel&&<section id="model" className="inspector"><span className="eyebrow">HOW THIS STORY IS BUILT</span><h2>Not a generated answer. A graph you can edit.</h2><div className="modelgrid"><p><b>places</b><br/>named locations with map coordinates.</p><p><b>consequences</b><br/>the scene, elapsed time, scale, editorial status, and links to possible next scenes.</p><p><b>choices</b><br/>each edge has a label and a causal explanation. the app follows only published records.</p></div><p className="small">The published scenario, places, scenes, and choice edges are read from Sanity. The app only shows scenes marked published.</p></section>}
  <section id="case" className="workspace"><div className="left"><div className="panehead"><span>01 / THE CITY</span><span>MINUTE {String(current.minutesLater).padStart(2,'0')}</span></div><div className="map"><div className="grid"/><div className="road r1"/><div className="road r2"/><div className="river"/>{story.places.map(p=><div className={'marker '+(p.id===current.place?'active':'')} key={p.id} style={{left:`${p.x}%`,top:`${p.y}%`}}><span>✳</span><small>{p.name}</small></div>)}<div className="maplabel">THE CITY OF ALMOSTS<br/>NOT TO SCALE</div></div><div className="mapfoot">CURRENT LOCATION <strong>{place.name} / {place.district}</strong></div></div>
